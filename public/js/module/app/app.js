var App = (function() {
	...
	module.requestDataTahap = () => {
		...
		$.ajax({
			...
			// console.log(response);
				...
				module.appendHistoryVerifikasi(response.data);
		});
	};
	...
	module.dataOrmasData = (data) => {
		...
		} else	if (dataNotifVerifyLapangan[0].tahap == "wawancara-yes" && dataNotifVerifyLapangan[0].notif == "send") {
				module.notifVerifySuccessForLapangan(dataForLapangan);
		} else if (dataNotifVerifyLapangan[0].tahap == "wawancara-no" && dataNotifVerifyLapangan[0].notif == "send") {
			module.notifVerifyGagalForLapangan(dataForLapangan);
		}	else if (dataNotifVerifyLapangan[0].tahap == "pengambilan-yes" && dataNotifVerifyLapangan[0].notif == "send") {
			module.notifVerifySuccessForLapangan(dataForLapangan);
		} else if (dataNotifVerifyLapangan[0].tahap == "pengambilan-no" && dataNotifVerifyLapangan[0].notif == "send") {
			module.notifVerifyGagalForLapangan(dataForLapangan);
		}
						...
						<div class='buttons'>
							<form class="" action="${ijin}/app/readnotif" method="post">
									...
									<input type="hidden" name="id" value="${notif.idpesan}">
										<button class='no btn btn-sm btn-info'>dismiss</button>
			...
			html:
				`<div>
					<div class='clearfix'>
						<div class='title' data-notify-html='title'/>
						<div class='buttons'>
							<button class='no btn btn-sm btn-success'>dismiss</button>
							<a href="../app/history" class="btn btn-success btn-sm">detail</a>
						</div>
					</div>
				</div>`
		});
			...
			html:
				`<div>
					<div class='clearfix'>
						<div class='title' data-notify-html='title'/>
						<div class='buttons'>
							<button class='no btn btn-sm btn-danger'>dismiss</button>
							<a href="../app/history" class="btn btn-danger btn-sm">detail</a>
						</div>
					</div>
				</div>`
		});
	...
	module.appendHistoryVerifikasi = (verify) => {
		const dataorganisasi = $.grep(verify,function(item, index){
				return item.iduser == iduser.data('id');
		});
		// console.log(dataorganisasi);
		if (dataorganisasi[0].tahap == "lapangan-yes") {
			$('#history-user').html(`
				<div class="alert alert-success" role="alert">
					<h4 class="alert-heading"><i class="fa fa-check"></i> Data Anda Sukses Verifikasi !</h4>
					<p>Anda dijadwalkan untuk verifikasi lapangan</p>
					<p class="mb-0">Tanggal ${dataorganisasi[0].pesan_sukses}</p>
				</div>
				`);
		} else if (dataorganisasi[0].tahap == "lapangan-no") {
			$('#history-user').html(`
				<div class="alert alert-danger" role="alert">
					<h4 class="alert-heading"><i class="fa fa-close"></i> Data gagal verifikasi !</h4>
					<p>${dataorganisasi[0].pesan_gagal}</p>
					<a href="../app/ubah/${dataorganisasi[0].noregistrasi}" class="btn btn-sm btn-danger">ubah data </a>
				</div>
				`);
		} else if (dataorganisasi[0].tahap == "wawancara-yes") {
			$('#history-user').html(`
				<div class="alert alert-success" role="alert">
					<h4 class="alert-heading"><i class="fa fa-check"></i> Data Anda Sukses Verifikasi !</h4>
					<p>Anda dijadwalkan untuk wawancara</p>
					<p class="mb-0">Tanggal ${dataorganisasi[0].pesan_sukses}</p>
				</div>
				`);
		} else if (dataorganisasi[0].tahap == "wawancara-no") {
			$('#history-user').html(`
				<div class="alert alert-danger" role="alert">
					<h4 class="alert-heading"><i class="fa fa-close"></i> Gagal verifikasi lapangan </h4>
					<p>${dataorganisasi[0].pesan_gagal}</p>
				</div>
				`);
			} else if (dataorganisasi[0].tahap == "pengambilan-yes") {
				$('#history-user').html(`
					<div class="alert alert-success" role="alert">
						<h4 class="alert-heading"><i class="fa fa-check"></i> Anda Sukses Wawancara !</h4>
						<p>Anda dijadwalkan untuk melakukan pengambilan SKT</p>
						<p class="mb-0">Tanggal ${dataorganisasi[0].pesan_sukses}</p>
					</div>
					`);
			}
